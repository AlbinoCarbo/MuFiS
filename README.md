# Introduction to MuFiS
MuFiS stands for "Multipurpose Fireball Software" and is a Matlab-based software for the triangulation, analysis of atmospheric dynamics, orbit determination and (as a first approximation), strewn field, of brilliant fireballs generated by meteoroids falling into the earth's atmosphere.

1. This software has been developed during the years 2017-2020 for the physical analysis of the bolides observed in the Earth's atmosphere within the italian [PRISMA Project](http://www.prisma.inaf.it/). A network consisting of 60 all-sky cameras for monitoring fireballs and collecting meteorites on the ground.
2. It is a software that runs under 64-bit Windows inside a DOS or PowerShell window, developed and compiled with Matlab R2015b.
3. There are two modes of use:
   
- BASIC: it is used for a first analysis of the fireball, when there are visual observations or when the speed data is noisy
- ADVANCED: it is used when the observations are digital and with low uncertainty or when a physical model of the meteoroid is required (mass, dimensions, ablation)
...
4. The "IT20000000" folder is supplied with MuFiS which contains the files with the input data for a simulated fireball observed from 4 fictitious stations.
5. Files are text in ANSI format. Other formats could generate reading errors in the data input phase. For the structure of the input files, Standard or PRISMA, see further on.
6. This simulated fireball, whose parameters are known a priori, was used to test MuFiS.
